...
//////////////////////////////////////////////////////////////////////////////
  ...
  // The implementation in this file is taken from rev 5add726b69131a2cf0ffd6b8ba6c08e5097cd4db
  // This version uses the logistic function to bound the cpuct term. It honors MaxCollisionVisits but does not separate evaluation from exploration
  // Results @12.000 nodes worse than baseline
...
float computeChildWeights(NodeGlow* node, bool evaluation_weights, int node_n) {
  ...
  for (NodeGlow *i = node->GetFirstChild(); i != nullptr; i = i->GetNextSibling(), ii++) {
    ...
    double relative_weight_of_p = pow(i->GetN(), param_fpuValue_false) / (0.05 + i->GetN()) + cpuct_as_prob; // 0.05 is here to make Q have some influence after 1 visit.
