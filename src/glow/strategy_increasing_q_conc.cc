...
inline float q_to_prob(const float q, const float max_q, const float q_concentration, int n, int parent_n) {
  ...
  case 1: {
    ...
    // } else {
    int max_n=1000000;
    float my_q_concentration =  parent_n / sqrt(parent_n) * q_concentration / (max_n/sqrt(max_n));
    return exp(my_q_concentration * (q - abs(max_q)/2)); // reduce the overflow risk. However, with the default q_concentration 36.2, overflow isn't possible since exp(36.2 * 1) is less than max float. TODO restrict the parameter so that it cannot overflow and remove this division.
